---
prompt_version: kg_poc_v1.3
description: KG-RAG PoC 評価用 QA 生成プロンプト (3軸タグ + 静的 Few-Shot + 場所当て/こじつけ禁止。難易度は後段で確定)
---
...
- 質問: 「6週間以内に出産予定の女性労働者が産前産後の休業を請求した場合、その休業中に業務外の傷病による欠勤が続いたら、休職への切り替えや復帰は規則上どのような扱いになるか。関連する条文を挙げて説明せよ。」
- 回答: 「産前産後の休業を請求した女性労働者は就業させてはいけない(第25条)。休業期間中に業務外の傷病による欠勤が所定期間を超えた場合は第9条により休職となり、療養が必要な間は休職を継続する。休職事由が消滅すれば原則元の職務に復帰する。」
- なぜ良いか: **第25条と第9条という離れた2つの条文を実際に辿らないと答えが組み立たない**。条文番号は答えの根拠として添えており、番号当て自体が問いではない。

**手本2 (traceability)**
- 質問: 「労働者の労働時間の状況を把握した後、面接指導を実施し、その結果に基づき事業者が講じなければならない措置と、面接指導の結果の記録保存の要件を、規定に従い追跡して説明せよ。」
- 回答: 「客観的な方法で労働時間の状況を把握し、該当する労働者の申出により医師による面接指導を行う。結果に基づき就業場所の変更、作業の転換、労働時間の短縮、深夜業の回数の減少等の措置を講じる。面接指導の結果は記録を作成し5年間保存する。」
- なぜ良いか: **把握→指導→措置→記録という規定上の手続きの連鎖**を順に追跡させており、1チャンクの抜き出しでは答えられない。
...
- ✗ **問い詰め文体の乱用**: 「〜を列挙せよ」「〜を述べよ」を機械的に積む長文。実務者(人事担当者・労働者)が現場で実際に聞く自然な質問文にする。
